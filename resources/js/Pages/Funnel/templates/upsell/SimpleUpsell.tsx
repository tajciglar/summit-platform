      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm p-8 text-center">
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={processing}
            className="w-full text-white font-semibold py-3 rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity"
            style={{ backgroundColor: 'var(--theme-primary)' }}
          >
            {processing ? 'Processing…' : content.cta_text ?? `Yes — Add for $${((product?.price_cents ?? 0) / 100).toFixed(2)}`}
          </button>
          <button
