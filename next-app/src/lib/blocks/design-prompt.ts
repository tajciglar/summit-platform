...
export async function buildDesignPrompt(input: BuildDesignPromptInput): Promise<DesignPrompt> {
  ...
  const text = [
    ...
    `- Only imports allowed: react, @/components/ui/*. Do NOT import from lucide-react or any other icon library.`,
    `- For icons, inline SVGs directly in the JSX. Use simple geometric SVGs (24x24 viewBox, currentColor stroke) — keep markup short.`,
