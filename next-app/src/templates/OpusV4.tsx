// Document chrome (html/head/body) is owned by the Next.js page/layout that
// renders this template. Fonts (Space Grotesk / Inter / DM Serif Display)
// must be loaded by the page — see Task 19/20 (preview/public routes).
import { OptinModal } from '@/components/OptinModal';
import type { OpusV4Content } from './opus-v4.schema';
import type { Speaker } from './types';

type Props = {
  content: OpusV4Content;
  speakers: Record<string, Speaker>;
};

type RootProps = Props & {
  funnelId: string;
};

// Deterministic sparkline heights keyed by trend label.
const TREND_HEIGHTS: Record<'rising' | 'plateau' | 'falling' | 'volatile', number[]> = {
  rising:   [40, 55, 72, 88, 100],
  plateau:  [88, 90, 92, 94, 96],
  falling:  [100, 85, 68, 50, 35],
  volatile: [55, 92, 40, 78, 62],
};

// Hero portrait card gradients — violet/sun/mist rotation per v4 HTML.
const HERO_CARD_GRADIENTS = [
  'linear-gradient(160deg,#4A2FB8,#23135F)',
  'linear-gradient(160deg,#FFC300,#B88C00)',
  'linear-gradient(160deg,#C6C1DB,#7E7399)',
  'linear-gradient(160deg,#8A6EEB,#5C3BDF)',
];
const HERO_CARD_NAME_COLORS = ['#FFFFFF', '#23135F', '#23135F', '#FFFFFF'];
const HERO_CARD_TITLE_COLORS = ['#C5B8F7', '#381F8E', '#381F8E', '#C5B8F7'];

// Hero bottom avatar strip gradients (mini 4-up).
const HERO_AVATAR_GRADIENTS = [
  'linear-gradient(135deg,#FFC300,#FFD347)',
  'linear-gradient(135deg,#8A6EEB,#4A2FB8)',
  'linear-gradient(135deg,#C6C1DB,#A59DC2)',
  'linear-gradient(135deg,#FFD347,#FFC300)',
];
const HERO_AVATAR_TEXT_COLORS = ['#4A2FB8', '#FFFFFF', '#4A2FB8', '#4A2FB8'];

// Speaker-day grid avatar gradients (8-up cycle).
const SPEAKER_GRADIENTS = [
  'linear-gradient(135deg,#6F4EE6,#4A2FB8)',
  'linear-gradient(135deg,#FFC300,#B88C00)',
  'linear-gradient(135deg,#8A6EEB,#5C3BDF)',
  'linear-gradient(135deg,#23135F,#4A2FB8)',
  'linear-gradient(135deg,#C6C1DB,#7E7399)',
  'linear-gradient(135deg,#4A2FB8,#23135F)',
  'linear-gradient(135deg,#FFD347,#FFC300)',
  'linear-gradient(135deg,#6F4EE6,#8A6EEB)',
];
const SPEAKER_INITIAL_COLORS = [
  '#FFFFFF',
  '#23135F',
  '#FFFFFF',
  '#FFC300',
  '#23135F',
  '#FFFFFF',
  '#23135F',
  '#FFFFFF',
];

// Founders avatars.
const FOUNDER_GRADIENTS = [
  'linear-gradient(135deg,#6F4EE6,#4A2FB8)',
  'linear-gradient(135deg,#FFC300,#FFD347)',
];
const FOUNDER_TEXT_COLORS = ['#FFFFFF', '#23135F'];

// Testimonial avatars.
const TESTIMONIAL_GRADIENTS = [
  'linear-gradient(135deg,#6F4EE6,#4A2FB8)',
  'linear-gradient(135deg,#FFC300,#FFD347)',
  'linear-gradient(135deg,#8A6EEB,#5C3BDF)',
];
const TESTIMONIAL_TEXT_COLORS = ['#FFFFFF', '#23135F', '#FFFFFF'];

// Outcome card icon bg toggle (white vs sun-400).
const OUTCOME_ICON_BGS = ['#FFFFFF', '#FFC300'];

// Figure value color (violet default; accent indexes get warm tones per HTML).
const FIGURE_VALUE_COLORS: Record<number, string> = {
  1: '#C4663D',
  4: '#B88C00',
};

function initialsFromSpeaker(s: Speaker): string {
  const a = s.firstName?.trim()?.[0] ?? '';
  const b = s.lastName?.trim()?.[0] ?? '';
  return (a + b).toUpperCase() || '??';
}

function displayName(s: Speaker): string {
  return `${s.firstName ?? ''} ${s.lastName ?? ''}`.trim() || '—';
}

/* ============== BRAND MARK ============== */
function BrandMark() {
  return (
    <svg width="30" height="30" viewBox="0 0 32 32" fill="none" aria-hidden="true">
      <rect x="2" y="2" width="28" height="28" rx="10" fill="#FFC300" />
      <path
        d="M10 21 L 14 13 L 18 19 L 22 11"
        stroke="#110833"
        strokeWidth="2.8"
        strokeLinecap="round"
        strokeLinejoin="round"
        fill="none"
      />
    </svg>
  );
}

/* ============== STICKY TOP BAR ============== */
function TopBar({ content }: { content: OpusV4Content }) {
  const t = content.topBar;
  return (
    <header
      className="sticky top-0 z-40 text-white"
      style={{ background: '#6F4EE6' }}
    >
      <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <BrandMark />
          <span className="opus-v4-display font-bold text-lg tracking-tight">
            {t.brandName}
          </span>
        </div>
        <div className="flex items-center gap-4">
          <span
            className="hidden md:inline-flex items-center gap-2 text-sm font-medium"
            style={{ color: '#E6E0FD' }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ background: '#FFC300' }}
            ></span>
            {t.dateLabel}
          </span>
          <a
            href="#optin"
            className="opus-v4-btn-sun text-sm"
            style={{ padding: '0.5rem 1.25rem', boxShadow: 'none' }}
          >
            {t.ctaLabel}
          </a>
        </div>
      </div>
    </header>
  );
}

/* ============== HERO ============== */
function Hero({ content, speakers }: Props) {
  const h = content.hero;
  const heroSpeakers = h.heroSpeakerIds
    .map((id) => speakers[id])
    .filter((s): s is Speaker => Boolean(s));

  return (
    <section className="relative opus-v4-grad-hero text-white overflow-hidden">
      <div className="absolute inset-0 opus-v4-dots-bg"></div>
      <div
        className="absolute -top-24 -right-24 w-[32rem] h-[32rem] rounded-full opacity-40"
        style={{
          background: 'radial-gradient(circle,#FFC300,transparent 65%)',
          filter: 'blur(40px)',
        }}
      ></div>
      <div
        className="absolute -bottom-32 -left-24 w-[28rem] h-[28rem] rounded-full opacity-50"
        style={{
          background: 'radial-gradient(circle,#C6C1DB,transparent 65%)',
          filter: 'blur(40px)',
        }}
      ></div>

      <div className="relative max-w-7xl mx-auto px-6 pt-16 md:pt-24 pb-20 md:pb-28">
        <div className="grid md:grid-cols-12 gap-10 items-center">
          <div className="md:col-span-7">
            <div
              className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 mb-6 backdrop-blur-sm"
              style={{
                background: 'rgba(255,255,255,0.1)',
                border: '1px solid rgba(255,255,255,0.2)',
              }}
            >
              <span
                className="w-2 h-2 rounded-full animate-pulse"
                style={{ background: '#FFC300' }}
              ></span>
              <span className="text-sm font-medium" style={{ color: '#E6E0FD' }}>
                {h.pillLabel}
              </span>
            </div>

            <h1 className="opus-v4-display font-bold text-5xl md:text-6xl lg:text-7xl leading-[1.02] tracking-[-0.03em] mb-7">
              <span className="opus-v4-italic-serif" style={{ color: '#FFD347' }}>
                {h.headlineAccent}
              </span>
              {h.headlineTrail}
            </h1>

            <p
              className="text-lg md:text-xl leading-[1.6] mb-9 max-w-xl"
              style={{ color: '#E6E0FD' }}
            >
              {h.subheadline}
            </p>

            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-10">
              <a
                href="#optin"
                className="opus-v4-btn-sun opus-v4-btn-sun-pulse"
              >
                {h.primaryCtaLabel}
                <span aria-hidden="true">→</span>
              </a>
              <a href="#what-is-this" className="opus-v4-btn-violet">
                {h.secondaryCtaLabel}
              </a>
            </div>

            <div
              className="flex items-center gap-4 pt-8"
              style={{ borderTop: '1px solid rgba(255,255,255,0.2)' }}
            >
              <div className="flex -space-x-3">
                {heroSpeakers.slice(0, 4).map((s, idx) => (
                  <div
                    key={`hero-avatar-${s.id}`}
                    className="w-11 h-11 rounded-full flex items-center justify-center opus-v4-display font-bold text-sm"
                    style={{
                      background:
                        HERO_AVATAR_GRADIENTS[idx % HERO_AVATAR_GRADIENTS.length],
                      color:
                        HERO_AVATAR_TEXT_COLORS[idx % HERO_AVATAR_TEXT_COLORS.length],
                      border: '2px solid #6F4EE6',
                    }}
                  >
                    {initialsFromSpeaker(s)}
                  </div>
                ))}
              </div>
              <div className="text-sm">
                <p className="opus-v4-stars mb-0.5">
                  ★★★★★{' '}
                  <span
                    className="ml-1 font-medium"
                    style={{ color: 'rgba(255,255,255,0.9)', letterSpacing: 'normal' }}
                  >
                    {h.ratingLabel}
                  </span>
                </p>
                <p style={{ color: '#E6E0FD' }}>
                  {h.readerCountLead}{' '}
                  <span className="font-semibold text-white">{h.readerCount}</span>{' '}
                  {h.readerCountSuffix}
                </p>
              </div>
            </div>
          </div>

          <div className="md:col-span-5">
            <div className="grid grid-cols-2 gap-4 relative">
              <span
                className="absolute -top-5 -right-5 w-20 h-20 rounded-full opus-v4-grad-button shadow-2xl flex items-center justify-center opus-v4-display font-bold text-center p-2"
                style={{
                  color: '#4A2FB8',
                  fontSize: '0.65rem',
                  lineHeight: '1.1',
                  transform: 'rotate(8deg)',
                }}
              >
                {h.freeBadge}
              </span>

              {heroSpeakers.slice(0, 4).map((s, idx) => {
                const marginTopClass =
                  idx === 1 ? 'mt-8' : idx === 2 ? '-mt-4' : idx === 3 ? 'mt-4' : '';
                return (
                  <div
                    key={`hero-card-${s.id}`}
                    className={`aspect-[3/4] rounded-3xl flex items-end justify-start p-5 shadow-2xl ${marginTopClass}`}
                    style={{
                      background:
                        HERO_CARD_GRADIENTS[idx % HERO_CARD_GRADIENTS.length],
                    }}
                  >
                    <div>
                      <p
                        className="opus-v4-display font-bold text-lg"
                        style={{
                          color:
                            HERO_CARD_NAME_COLORS[idx % HERO_CARD_NAME_COLORS.length],
                        }}
                      >
                        {displayName(s)}
                      </p>
                      {s.title ? (
                        <p
                          className="text-xs mt-1"
                          style={{
                            color:
                              HERO_CARD_TITLE_COLORS[
                                idx % HERO_CARD_TITLE_COLORS.length
                              ],
                          }}
                        >
                          {s.title}
                        </p>
                      ) : null}
                    </div>
                  </div>
                );
              })}
            </div>
            <p
              className="text-center text-xs mt-5 font-medium"
              style={{ color: 'rgba(230,224,253,0.8)' }}
            >
              {h.moreLabel}
            </p>
          </div>
        </div>
      </div>

      {/* Wave divider to white */}
      <svg
        className="block absolute -bottom-px left-0 right-0 w-full h-8 md:h-12"
        viewBox="0 0 1200 40"
        preserveAspectRatio="none"
        fill="#FFFFFF"
        aria-hidden="true"
      >
        <path d="M0 40 Q 300 0, 600 20 T 1200 10 L 1200 40 L 0 40 Z" />
      </svg>
    </section>
  );
}

/* ============== PRESS MARQUEE ============== */
function Press({ content }: { content: OpusV4Content }) {
  const items = [...content.press.outlets, ...content.press.outlets];
  return (
    <section className="bg-white py-10">
      <div className="max-w-7xl mx-auto px-6">
        <p
          className="opus-v4-eyebrow text-center mb-6"
          style={{ color: '#7E7399' }}
        >
          {content.press.eyebrow}
        </p>
        <div className="opus-v4-marquee-wrap">
          <div className="opus-v4-marquee-track">
            {items.map((name, idx) => (
              <span className="opus-v4-marquee-item" key={`press-${idx}`}>
                {name}
              </span>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}

/* ============== STATS ============== */
function Stats({ content }: { content: OpusV4Content }) {
  return (
    <section className="opus-v4-grad-mist py-16 md:py-20">
      <div className="max-w-6xl mx-auto px-6">
        <p
          className="opus-v4-eyebrow text-center mb-10"
          style={{ color: '#2A1869' }}
        >
          {content.stats.eyebrow}
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-4 text-center">
          {content.stats.items.map((item, idx) => {
            const isLast = idx === content.stats.items.length - 1;
            const borderClass = isLast ? '' : 'md:border-r md:pr-4';
            return (
              <div
                key={`stat-${idx}`}
                className={borderClass}
                style={{
                  borderColor: isLast ? undefined : 'rgba(138,110,235,0.3)',
                }}
              >
                <p
                  className="opus-v4-display font-bold text-7xl md:text-8xl leading-none mb-3 tracking-[-0.04em]"
                  style={{ color: '#2A1869' }}
                >
                  {item.value}
                  {item.suffix ? (
                    <span style={{ color: '#FFC300' }}>{item.suffix}</span>
                  ) : null}
                </p>
                <p
                  className="text-lg font-medium"
                  style={{ color: '#544B75' }}
                >
                  {item.description}
                </p>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
}

/* ============== OVERVIEW ============== */
function Overview({ content }: { content: OpusV4Content }) {
  const o = content.overview;
  return (
    <section id="what-is-this" className="bg-white py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-12 gap-12 items-center">
        <div className="md:col-span-7">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#6F4EE6' }}
          >
            {o.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-6xl leading-[1.02] tracking-[-0.03em] mb-7"
            style={{ color: '#110833' }}
          >
            {o.headlineLead}
            <span className="opus-v4-hl-sun">{o.headlineHighlight}</span>
            {o.headlineMid}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#6F4EE6' }}
            >
              {o.headlineAccent}
            </span>
            {o.headlineTrail}
          </h2>
          {o.bodyParagraphs.map((para, idx) => {
            const isLast = idx === o.bodyParagraphs.length - 1;
            return (
              <p
                key={`overview-p-${idx}`}
                className={`text-lg md:text-xl leading-[1.65] ${isLast ? 'mb-9' : 'mb-5'}`}
                style={{ color: '#544B75' }}
              >
                {para}
              </p>
            );
          })}
          <a href="#optin" className="opus-v4-btn-sun">
            {o.ctaLabel}
            <span aria-hidden="true">→</span>
          </a>
        </div>
        <div className="md:col-span-5">
          <div className="opus-v4-card-mist p-8">
            <p
              className="opus-v4-eyebrow mb-6"
              style={{ color: '#2A1869' }}
            >
              {o.cardEyebrow}
            </p>
            <div className="space-y-5">
              {o.components.map((comp, idx) => {
                const iconBg = idx % 2 === 0 ? '#2A1869' : '#FFC300';
                return (
                  <div key={`overview-comp-${idx}`} className="flex gap-4">
                    <div
                      className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0"
                      style={{ background: iconBg }}
                    >
                      <span
                        className="opus-v4-display font-bold text-base"
                        style={{ color: idx % 2 === 0 ? '#FFC300' : '#110833' }}
                      >
                        {String(idx + 1).padStart(2, '0')}
                      </span>
                    </div>
                    <div>
                      <p
                        className="opus-v4-display font-bold text-lg"
                        style={{ color: '#110833' }}
                      >
                        {comp.title}
                      </p>
                      <p
                        className="text-sm mt-1"
                        style={{ color: '#544B75' }}
                      >
                        {comp.description}
                      </p>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

/* ============== SPEAKER GRID — DAY 1 ============== */
function SpeakersDay({ content, speakers }: Props) {
  const s = content.speakersDay;
  const daySpeakers = s.speakerIds
    .map((id) => speakers[id])
    .filter((sp): sp is Speaker => Boolean(sp));

  return (
    <section className="opus-v4-grad-mist py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-6">
          <div>
            <span
              className="inline-block opus-v4-eyebrow px-4 py-2 rounded-full mb-5"
              style={{ background: '#6F4EE6', color: '#FFFFFF' }}
            >
              {s.dayLabel}
            </span>
            <h2
              className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
              style={{ color: '#110833' }}
            >
              {s.headlineLead}
              <span
                className="opus-v4-italic-serif"
                style={{ color: '#2A1869' }}
              >
                {s.headlineAccent}
              </span>
              {s.headlineTrail}
            </h2>
          </div>
          <p className="font-medium" style={{ color: '#544B75' }}>
            {s.countLabel}
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-6">
          {daySpeakers.map((sp, idx) => (
            <figure key={sp.id} className="opus-v4-card-light p-6 text-center">
              <div
                className="w-28 h-28 rounded-full mx-auto mb-4 flex items-end justify-center pb-4"
                style={{
                  background:
                    SPEAKER_GRADIENTS[idx % SPEAKER_GRADIENTS.length],
                }}
              >
                <span
                  className="opus-v4-display font-bold text-3xl"
                  style={{
                    color:
                      SPEAKER_INITIAL_COLORS[idx % SPEAKER_INITIAL_COLORS.length],
                  }}
                >
                  {initialsFromSpeaker(sp)}
                </span>
              </div>
              <p
                className="opus-v4-display font-bold"
                style={{ color: '#110833' }}
              >
                {displayName(sp)}
              </p>
              {sp.title ? (
                <p className="text-sm mt-1" style={{ color: '#6B638A' }}>
                  {sp.title}
                </p>
              ) : null}
            </figure>
          ))}
        </div>

        <div className="text-center mt-12">
          <a href="#optin" className="opus-v4-btn-outline">
            {s.ctaLabel}
          </a>
        </div>
      </div>
    </section>
  );
}

/* ============== OUTCOMES ============== */
function Outcomes({ content }: { content: OpusV4Content }) {
  const o = content.outcomes;
  const total = o.items.length;
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-14">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#6F4EE6' }}
          >
            {o.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
            style={{ color: '#110833' }}
          >
            {o.headlineLead}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#6F4EE6' }}
            >
              {o.headlineAccent}
            </span>
          </h2>
        </div>
        <div className="grid md:grid-cols-3 gap-5">
          {o.items.map((item, idx) => {
            const iconBg = OUTCOME_ICON_BGS[idx % OUTCOME_ICON_BGS.length];
            return (
              <article key={`outcome-${idx}`} className="opus-v4-card-mist p-7">
                <div className="flex items-center justify-between mb-5">
                  <div
                    className="w-12 h-12 rounded-2xl flex items-center justify-center"
                    style={{ background: iconBg }}
                  >
                    <span
                      className="opus-v4-display font-bold text-lg"
                      style={{
                        color: iconBg === '#FFFFFF' ? '#2A1869' : '#23135F',
                      }}
                    >
                      {String(idx + 1).padStart(2, '0')}
                    </span>
                  </div>
                  <span
                    className="opus-v4-display font-bold text-sm"
                    style={{ color: '#2A1869' }}
                  >
                    {String(idx + 1).padStart(2, '0')} / {String(total).padStart(2, '0')}
                  </span>
                </div>
                <h3
                  className="opus-v4-display font-bold text-xl mb-2"
                  style={{ color: '#110833' }}
                >
                  {item.title}
                </h3>
                <p
                  className="leading-relaxed"
                  style={{ color: 'rgba(35,19,95,0.7)' }}
                >
                  {item.description}
                </p>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}

/* ============== FREE GIFT ============== */
function FreeGift({ content }: { content: OpusV4Content }) {
  const g = content.freeGift;
  return (
    <section className="opus-v4-grad-violet-dark text-white py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 opus-v4-dots-bg opacity-40"></div>
      <div
        className="absolute -top-16 -right-16 w-96 h-96 rounded-full opacity-30"
        style={{
          background: 'radial-gradient(circle,#FFC300,transparent 65%)',
          filter: 'blur(40px)',
        }}
      ></div>

      <div className="relative max-w-6xl mx-auto px-6 grid md:grid-cols-12 gap-12 items-center">
        <div className="md:col-span-5 flex justify-center">
          <div className="relative">
            <div
              className="w-64 h-80 rounded-3xl shadow-2xl p-7 flex flex-col justify-between"
              style={{
                background: '#FFFFFF',
                transform: 'rotate(-3deg)',
              }}
            >
              <div>
                <div className="w-full h-2 rounded-full opus-v4-grad-button mb-4"></div>
                <div className="w-12 h-12 rounded-2xl opus-v4-grad-button mb-4 flex items-center justify-center">
                  <svg
                    className="w-6 h-6"
                    style={{ color: '#23135F' }}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2.2"
                      d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"
                    />
                  </svg>
                </div>
                <p
                  className="opus-v4-display font-bold text-lg leading-tight"
                  style={{ color: '#23135F' }}
                >
                  {g.cardTitle}
                </p>
              </div>
              <div>
                <p className="text-xs" style={{ color: '#7E7399' }}>
                  {g.cardSubtitle}
                </p>
              </div>
            </div>
            <div
              className="absolute -top-4 -right-4 opus-v4-display font-bold text-xs px-4 py-2 rounded-full shadow-2xl"
              style={{
                background: '#FFC300',
                color: '#23135F',
                transform: 'rotate(6deg)',
              }}
            >
              {g.cardBadge}
            </div>
          </div>
        </div>
        <div className="md:col-span-7">
          <span
            className="opus-v4-eyebrow mb-4 inline-block"
            style={{ color: '#FFC300' }}
          >
            {g.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl leading-tight mb-6 tracking-[-0.03em]"
          >
            {g.headlineLead}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#FFD347' }}
            >
              {g.headlineAccent}
            </span>
            {g.headlineTrail}
          </h2>
          <p
            className="text-lg leading-relaxed mb-7"
            style={{ color: '#E6E0FD' }}
          >
            {g.body}
          </p>
          <ul className="space-y-3 mb-8">
            {g.bullets.map((bullet, idx) => (
              <li
                key={`gift-bullet-${idx}`}
                className="flex items-start gap-3"
              >
                <span
                  className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5"
                  style={{ background: '#FFC300' }}
                >
                  <span
                    className="font-bold"
                    style={{ color: '#23135F' }}
                  >
                    ✓
                  </span>
                </span>
                <span style={{ color: '#E6E0FD' }}>{bullet}</span>
              </li>
            ))}
          </ul>
          <a href="#optin" className="opus-v4-btn-sun">
            {g.ctaLabel}
            <span aria-hidden="true">→</span>
          </a>
        </div>
      </div>
    </section>
  );
}

/* ============== BONUSES ============== */
function Bonuses({ content }: { content: OpusV4Content }) {
  const b = content.bonuses;
  return (
    <section className="opus-v4-grad-mist py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-14">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#2A1869' }}
          >
            {b.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
            style={{ color: '#110833' }}
          >
            <span className="opus-v4-hl-sun">{b.headlineHighlight}</span>
            {b.headlineTrail}
          </h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {b.items.map((bonus, idx) => (
            <article key={`bonus-${idx}`} className="opus-v4-card-light p-8">
              <div className="flex items-center justify-between mb-5">
                <span
                  className="opus-v4-eyebrow"
                  style={{ color: '#6F4EE6' }}
                >
                  {bonus.label}
                </span>
                <span
                  className="opus-v4-eyebrow px-3 py-1.5 rounded-full"
                  style={{ background: '#FFC300', color: '#23135F' }}
                >
                  {bonus.valueLabel}
                </span>
              </div>
              <h3
                className="opus-v4-display font-bold text-2xl mb-3 leading-tight"
                style={{ color: '#110833' }}
              >
                {bonus.title}
              </h3>
              <p
                className="mb-5 leading-relaxed"
                style={{ color: '#544B75' }}
              >
                {bonus.description}
              </p>
              <ul className="space-y-2 text-sm" style={{ color: '#2A1869' }}>
                {bonus.bullets.map((bullet, bIdx) => (
                  <li
                    key={`bonus-${idx}-b-${bIdx}`}
                    className="flex gap-2"
                  >
                    <span
                      className="font-bold"
                      style={{ color: '#6F4EE6' }}
                    >
                      ✓
                    </span>
                    {bullet}
                  </li>
                ))}
              </ul>
            </article>
          ))}
        </div>
        <div className="text-center mt-12">
          <a
            href="#optin"
            className="opus-v4-btn-sun opus-v4-btn-sun-pulse"
          >
            {b.ctaLabel}
            <span aria-hidden="true">→</span>
          </a>
        </div>
      </div>
    </section>
  );
}

/* ============== FOUNDERS ============== */
function Founders({ content }: { content: OpusV4Content }) {
  const f = content.founders;
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="max-w-5xl mx-auto px-6">
        <h2
          className="opus-v4-display font-bold text-4xl md:text-5xl text-center mb-14 tracking-[-0.03em] leading-tight"
          style={{ color: '#110833' }}
        >
          {f.headlineLead}
          <span
            className="opus-v4-italic-serif"
            style={{ color: '#6F4EE6' }}
          >
            {f.headlineAccent}
          </span>
        </h2>
        <div className="grid md:grid-cols-2 gap-12">
          {f.items.map((founder, idx) => (
            <div
              key={`founder-${idx}`}
              className="opus-v4-card-mist p-8 md:p-10"
            >
              <div className="flex items-center gap-4 mb-6">
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center opus-v4-display font-bold text-xl shadow-lg"
                  style={{
                    background:
                      FOUNDER_GRADIENTS[idx % FOUNDER_GRADIENTS.length],
                    color:
                      FOUNDER_TEXT_COLORS[idx % FOUNDER_TEXT_COLORS.length],
                  }}
                >
                  {founder.initials}
                </div>
                <div>
                  <p
                    className="opus-v4-display font-bold text-lg"
                    style={{ color: '#110833' }}
                  >
                    {founder.name}
                  </p>
                  <p className="text-sm" style={{ color: '#6B638A' }}>
                    {founder.role}
                  </p>
                </div>
              </div>
              <blockquote
                className="opus-v4-italic-serif text-xl leading-relaxed pl-6"
                style={{
                  color: 'rgba(35,19,95,0.9)',
                  borderLeft: '4px solid #FFC300',
                }}
              >
                &ldquo;{founder.quote}&rdquo;
              </blockquote>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}

/* ============== TESTIMONIALS ============== */
function Testimonials({ content }: { content: OpusV4Content }) {
  const t = content.testimonials;
  return (
    <section className="opus-v4-grad-mist py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-14">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#2A1869' }}
          >
            {t.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
            style={{ color: '#110833' }}
          >
            {t.headlineLead}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#2A1869' }}
            >
              {t.headlineAccent}
            </span>
          </h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {t.items.map((item, idx) => (
            <article key={`testimonial-${idx}`} className="opus-v4-card-light p-8">
              <p className="opus-v4-stars mb-4">★★★★★</p>
              <p
                className="text-lg leading-relaxed mb-6"
                style={{ color: '#1E0F52' }}
              >
                &ldquo;{item.quote}&rdquo;
              </p>
              <div
                className="flex items-center gap-3 pt-5"
                style={{ borderTop: '1px solid #DCD7E6' }}
              >
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center opus-v4-display font-bold text-sm"
                  style={{
                    background:
                      TESTIMONIAL_GRADIENTS[idx % TESTIMONIAL_GRADIENTS.length],
                    color:
                      TESTIMONIAL_TEXT_COLORS[
                        idx % TESTIMONIAL_TEXT_COLORS.length
                      ],
                  }}
                >
                  {item.initials}
                </div>
                <div>
                  <p
                    className="opus-v4-display font-bold"
                    style={{ color: '#110833' }}
                  >
                    {item.name}
                  </p>
                  <p className="text-xs" style={{ color: '#6B638A' }}>
                    {item.location}
                  </p>
                </div>
              </div>
            </article>
          ))}
        </div>
      </div>
    </section>
  );
}

/* ============== PULL QUOTE ============== */
function PullQuote({ content }: { content: OpusV4Content }) {
  const pq = content.pullQuote;
  return (
    <section className="opus-v4-grad-violet-dark text-white py-24 md:py-32 relative overflow-hidden">
      <div className="absolute inset-0 opus-v4-dots-bg opacity-30"></div>
      <div
        className="absolute top-10 right-10 w-32 h-32 rounded-full blur-3xl"
        style={{ background: 'rgba(255,195,0,0.2)' }}
      ></div>
      <div className="relative max-w-4xl mx-auto px-6 text-center">
        <svg
          className="w-16 h-16 mx-auto mb-6"
          style={{ color: '#FFC300' }}
          fill="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H14.017zM0 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151C7.546 6.068 5.983 8.789 5.983 11H10v10H0z" />
        </svg>
        <p className="opus-v4-display font-bold text-3xl md:text-5xl leading-[1.15] mb-8 tracking-[-0.02em]">
          &ldquo;{pq.quote}&rdquo;
        </p>
        <div className="inline-flex items-center gap-4">
          <span
            className="w-12 h-[1px]"
            style={{ background: '#FFC300' }}
          ></span>
          <p className="opus-v4-eyebrow" style={{ color: '#FFC300' }}>
            {pq.attribution}
          </p>
          <span
            className="w-12 h-[1px]"
            style={{ background: '#FFC300' }}
          ></span>
        </div>
      </div>
    </section>
  );
}

/* ============== FIGURES / WHY THIS MATTERS ============== */
function Figures({ content }: { content: OpusV4Content }) {
  const f = content.figures;
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-6">
        <div className="max-w-2xl mb-14">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#6F4EE6' }}
          >
            {f.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
            style={{ color: '#110833' }}
          >
            {f.headlineLead}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#6F4EE6' }}
            >
              {f.headlineAccent}
            </span>
            {f.headlineTrail}
          </h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
          {f.items.map((item, idx) => {
            const valueColor = FIGURE_VALUE_COLORS[idx] ?? '#2A1869';
            return (
              <div key={`figure-${idx}`} className="opus-v4-card-mist p-7">
                <div className="flex items-center justify-between mb-3">
                  <span
                    className="opus-v4-eyebrow"
                    style={{ color: '#2A1869' }}
                  >
                    {item.label}
                  </span>
                  <div className="opus-v4-spark" aria-hidden="true">
                    {TREND_HEIGHTS[item.trend].map((h, hIdx) => (
                      <span
                        key={`spark-${idx}-${hIdx}`}
                        style={{ height: `${h}%` }}
                      ></span>
                    ))}
                  </div>
                </div>
                <p
                  className="opus-v4-display font-bold text-5xl mb-2 leading-none tracking-tight"
                  style={{ color: valueColor }}
                >
                  {item.value}
                </p>
                <p
                  className="text-sm leading-relaxed"
                  style={{ color: 'rgba(35,19,95,0.7)' }}
                >
                  {item.description}
                </p>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
}

/* ============== SHIFTS ============== */
function Shifts({ content }: { content: OpusV4Content }) {
  const s = content.shifts;
  return (
    <section className="opus-v4-grad-mist py-20 md:py-28">
      <div className="max-w-4xl mx-auto px-6">
        <span
          className="opus-v4-eyebrow mb-3 inline-block"
          style={{ color: '#2A1869' }}
        >
          {s.eyebrow}
        </span>
        <h2
          className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight mb-14"
          style={{ color: '#110833' }}
        >
          {s.headlineLead}
          <span
            className="opus-v4-italic-serif"
            style={{ color: '#2A1869' }}
          >
            {s.headlineAccent}
          </span>
        </h2>
        <div className="space-y-5">
          {s.items.map((item, idx) => (
            <article
              key={`shift-${idx}`}
              className="opus-v4-card-light p-7 flex gap-6 items-start"
            >
              <span
                className="w-14 h-14 rounded-2xl opus-v4-grad-button flex items-center justify-center opus-v4-display font-bold text-xl shrink-0"
                style={{ color: '#23135F' }}
              >
                {String(idx + 1).padStart(2, '0')}
              </span>
              <div>
                <h3
                  className="opus-v4-display font-bold text-xl mb-2"
                  style={{ color: '#110833' }}
                >
                  {item.title}
                </h3>
                <p
                  className="leading-relaxed"
                  style={{ color: '#544B75' }}
                >
                  {item.description}
                </p>
              </div>
            </article>
          ))}
        </div>
      </div>
    </section>
  );
}

/* ============== FAQ ============== */
function FAQ({ content }: { content: OpusV4Content }) {
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-12">
          <span
            className="opus-v4-eyebrow mb-3 inline-block"
            style={{ color: '#6F4EE6' }}
          >
            {content.faqSection.eyebrow}
          </span>
          <h2
            className="opus-v4-display font-bold text-4xl md:text-5xl tracking-[-0.03em] leading-tight"
            style={{ color: '#110833' }}
          >
            {content.faqSection.headlineLead}
            <span
              className="opus-v4-italic-serif"
              style={{ color: '#6F4EE6' }}
            >
              {content.faqSection.headlineAccent}
            </span>
          </h2>
        </div>
        {content.faqs.map((faq, idx) => (
          <details key={`faq-${idx}`}>
            <summary>
              {faq.question}
              <span className="opus-v4-pm-icon">+</span>
            </summary>
            <p
              className="px-7 pb-6 leading-relaxed"
              style={{ color: '#544B75' }}
            >
              {faq.answer}
            </p>
          </details>
        ))}
      </div>
    </section>
  );
}

/* ============== FINAL CTA ============== */
function FinalCTA({ content }: { content: OpusV4Content }) {
  const c = content.closing;
  return (
    <section
      id="final-cta"
      className="relative opus-v4-grad-hero text-white py-24 md:py-32 overflow-hidden"
    >
      <div className="absolute inset-0 opus-v4-dots-bg"></div>
      <div
        className="absolute -top-20 -left-20 w-[32rem] h-[32rem] rounded-full opacity-30"
        style={{
          background: 'radial-gradient(circle,#FFC300,transparent 65%)',
          filter: 'blur(40px)',
        }}
      ></div>
      <div
        className="absolute -bottom-20 -right-20 w-[30rem] h-[30rem] rounded-full opacity-40"
        style={{
          background: 'radial-gradient(circle,#C6C1DB,transparent 65%)',
          filter: 'blur(40px)',
        }}
      ></div>

      <div className="relative max-w-4xl mx-auto px-6 text-center">
        <span
          className="opus-v4-eyebrow mb-6 inline-block"
          style={{ color: '#FFC300' }}
        >
          {c.eyebrow}
        </span>
        <h2 className="opus-v4-display font-bold text-5xl md:text-6xl lg:text-7xl leading-[1.03] tracking-[-0.035em] mb-8">
          {c.headlineLead}
          <span
            className="opus-v4-italic-serif"
            style={{ color: '#FFD347' }}
          >
            {c.headlineAccent}
          </span>
          {c.headlineTrail}
        </h2>
        <p
          className="text-lg md:text-xl mb-10 max-w-2xl mx-auto leading-relaxed"
          style={{ color: '#E6E0FD' }}
        >
          {c.subheadline}
        </p>
        <a
          href="#optin"
          className="opus-v4-btn-sun opus-v4-btn-sun-pulse"
          style={{ fontSize: '1.125rem', padding: '1.25rem 2.5rem' }}
        >
          {c.ctaLabel}
          <span aria-hidden="true">→</span>
        </a>
        {c.fineprint ? (
          <p
            className="opus-v4-eyebrow mt-8"
            style={{ color: 'rgba(230,224,253,0.7)' }}
          >
            {c.fineprint}
          </p>
        ) : null}
      </div>
    </section>
  );
}

/* ============== FOOTER ============== */
function Footer({ content }: { content: OpusV4Content }) {
  const f = content.footer;
  return (
    <footer
      className="text-white py-14"
      style={{ background: '#110833' }}
    >
      <div className="max-w-7xl mx-auto px-6">
        <div
          className="grid md:grid-cols-[2fr_1fr_1fr_1fr] gap-10 pb-10"
          style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}
        >
          <div>
            <div className="flex items-center gap-3 mb-4">
              <BrandMark />
              <p className="opus-v4-display font-bold text-xl">
                {f.brandName}
              </p>
            </div>
            <p
              className="text-sm max-w-xs"
              style={{ color: '#C5B8F7' }}
            >
              {f.tagline}
            </p>
          </div>
          <div>
            <p
              className="opus-v4-eyebrow mb-4"
              style={{ color: '#A08CEF' }}
            >
              {f.summitLinksLabel}
            </p>
            <ul
              className="space-y-2 text-sm"
              style={{ color: 'rgba(230,224,253,0.8)' }}
            >
              {f.summitLinks.map((link, idx) => (
                <li key={`summit-link-${idx}`}>
                  <a href={link.href} className="hover:text-[#FFC300]">
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p
              className="opus-v4-eyebrow mb-4"
              style={{ color: '#A08CEF' }}
            >
              {f.legalLinksLabel}
            </p>
            <ul
              className="space-y-2 text-sm"
              style={{ color: 'rgba(230,224,253,0.8)' }}
            >
              {f.legalLinks.map((link, idx) => (
                <li key={`legal-link-${idx}`}>
                  <a href={link.href} className="hover:text-[#FFC300]">
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p
              className="opus-v4-eyebrow mb-4"
              style={{ color: '#A08CEF' }}
            >
              {f.contactLabel}
            </p>
            <ul
              className="space-y-2 text-sm"
              style={{ color: 'rgba(230,224,253,0.8)' }}
            >
              <li>{f.contactEmail}</li>
              <li style={{ color: 'rgba(255,255,255,0.4)' }}>
                {f.copyright}
              </li>
            </ul>
          </div>
        </div>
        <p
          className="text-xs text-center mt-6"
          style={{ color: 'rgba(197,184,247,0.6)' }}
        >
          {f.signoff}
        </p>
      </div>
    </footer>
  );
}

/* ============== ROOT COMPONENT ============== */
export function OpusV4({ content, speakers, funnelId }: RootProps) {
  return (
    <div className="opus-v4-root opus-v4-body antialiased">
      <a href="#main" className="opus-v4-skip-nav">
        Skip to content
      </a>

      <TopBar content={content} />

      <main id="main">
        <Hero content={content} speakers={speakers} />
        <Press content={content} />
        <Stats content={content} />
        <Overview content={content} />
        <SpeakersDay content={content} speakers={speakers} />
        <Outcomes content={content} />
        <FreeGift content={content} />
        <Bonuses content={content} />
        <Founders content={content} />
        <Testimonials content={content} />
        <PullQuote content={content} />
        <Figures content={content} />
        <Shifts content={content} />
        <FAQ content={content} />
        <FinalCTA content={content} />
      </main>

      <Footer content={content} />

      <OptinModal funnelId={funnelId} ctaLabel={content.hero.primaryCtaLabel} />
    </div>
  );
}
