...
export const IndigoGoldSchema = z.object({
  ...
  // Speakers render per-day automatically: the grid is driven entirely by
  // `distinct(speaker.day_number)` on the summit's Speaker rows. No per-day
  // config lives in page_content — adding a speaker with a new `day_number`
  // makes a new day block appear without touching the funnel. Labels are
  // generated by the renderer ("DAY 1", "DAY 2", …).
  outcomes: z.object({
