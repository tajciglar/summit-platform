...
import { OchreInkLayout } from './layout';
import { opusV1DefaultEnabledSections } from './sections';
import { ochreInkFixture } from '../__fixtures__/ochre-ink.fixture';
import type { Palette } from '@/lib/palette';

function render(props: React.ComponentProps<typeof OchreInkLayout>) {
  return renderToStaticMarkup(<OchreInkLayout {...props} />);
}

describe('OchreInkLayout', () => {
  it('renders default enabled sections', () => {
    const html = render({
      content: ochreInkFixture,
      speakers: {},
      funnelId: 'funnel-1',
    });
    expect(html).toContain('ochre-ink-root');
    expect(html).toContain(ochreInkFixture.hero.headline);
  });
  ...
  it('omits a section when enabledSections excludes it', () => {
    ...
    const dayLabel = ochreInkFixture.speakersByDay[0]?.dayLabel;
  ...
  it('omits palette vars when palette is null', () => {
    ...
    // the root entirely (so no `--primary:` or `style=` shows up on .ochre-ink-root).
    // Match the opening tag: `<div class="ochre-ink-root ..."` with no style attr.
    expect(html).toMatch(/<div class="ochre-ink-root[^"]*"(?!\s+style)/);
